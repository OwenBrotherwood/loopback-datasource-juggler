...
Validatable.prototype.isValid = function (callback, data) {
  ...
  var reportDiscardedProperties = this.__strict === 'validate' &&
    this.__discardedProperties && this.__discardedProperties.length;
  ...
  this.trigger('validate', function (validationsDone) {
    ...
    if (reportDiscardedProperties) {
      for (var ix in inst.__discardedProperties) {
        var key = inst.__discardedProperties[ix];
        inst.errors.add(key, defaultMessages['absence'], 'absence');
        valid = false;
      }
    }
